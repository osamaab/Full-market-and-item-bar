...
class ItemsCoordinator: NavigationCoordinator<ItemsRoute> {
    ...
    override func prepareTransition(for route: RouteType) -> TransitionType {
        ...
        case .items(let subcategory):
            let viewController = ItemsViewController(router: self.unownedRouter, category: subcategory)
            return .push(viewController)
        ...
        case .details(let product):
            return .push(CompanyProductDetailsViewController(product: product, router: self.unownedRouter))
        ...
        case .newQuantity(let product):
            NewProductQuantityViewController(product: product, delegate: self).present()
            return .none()
        }
    }
