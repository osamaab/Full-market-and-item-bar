...
extension WeekdaysRangePickerTextField {
    func workingDays(with startTime: String?, endTime: String?) -> [WorkingDayInput]? {
        guard let endWeekday = self.selectedEndWeekday,
              let startTime = startTime,
              let endTime = endTime else {
            return nil
        }
        
        let startWeekday = self.startingWeekday
        
         return (startWeekday.rawValue...endWeekday.rawValue).compactMap { Weekday(rawValue: $0) }.map {
            WorkingDayInput(dayId: $0.rawValue, timeFrom: startTime, timeTo: endTime)
        }
    }
}
