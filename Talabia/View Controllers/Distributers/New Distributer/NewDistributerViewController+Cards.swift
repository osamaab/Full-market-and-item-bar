...
class NDPersonalInformationContentView: BasicViewWithSetup {
    ...
    let mobileTextField = NumbersTextField(maxDigitsCount: 10, allowsDicimals: false)
...
class NDDeliveryAreaContentView: BasicViewWithSetup {
    ...
    let deliveryAreaView = DeliveryAreaInputView()
    
    override func setup() {
        addSubview(deliveryAreaView)
        deliveryAreaView.translatesAutoresizingMaskIntoConstraints = false
        deliveryAreaView.fillContainer()
    }
}
...
class NDPaymentMethodPickerContentView: CHPaymentMethodCardView {
