...
class DistributorSignUpViewController: ContentViewController<[CarType]> {
    ...
    fileprivate func connectActions(){
        ...
        bottomNextView.nextButton.add(event: .touchUpInside){ [unowned self] in
            ...
            performTask(taskOperation: AuthenticationAPI.distributorRegister(form).request(Distributor.self)).then { distributor in
                
                self.delegate?.distributorSignUpViewController(self, didFinishWith: distributor)
            }
        }
    }
